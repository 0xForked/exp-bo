    ...
    <div class="flex h-full">
      <div class="flex-2 h-full overflow-auto px-8">
        <div class="hidden lg:block heading flex-2">
          <h1 class="text-xl text-gray-700 mb-4">Conversation List ({{ chat.found_chats }})</h1>
        </div>
        
        <div v-for="msg in chat.chats_summary" :key="msg.id" class="entry cursor-pointer transform hover:scale-105 duration-300 transition-transform bg-white mb-4 rounded p-4 flex shadow-md border-l-4 border-red-500">
          <div class="flex-2">
          ...
          <div class="flex-1 px-2">
              <div class="truncate w-32">
                <span class="text-gray-800">{{ msg.last_event_per_type.filled_form.event.fields[0].answer }}</span>
              </div>
              <div>
                <small class="text-gray-600">{{ msg.last_event_per_type.message.event.text }}</small>
              </div>
          </div>
          <div class="flex-2 text-right">
            <div>
              <small class="text-gray-500">{{ myDateFormat(msg.last_event_per_type.filled_form.event.created_at) }}</small>
            </div>
          </div>
        </div>
      </div>
...
export default {
  ...
  async fetch() {
    this.chat = await await this.$axios.$get(`${apiUrl()}/livechat/agent`)
  },
  methods: {
    myDateFormat(d) {
